...
@interface LuckyLandOldSsoSetViewController () <GCDAsyncUdpSocketDelegate>
...
@implementation LuckyLandOldSsoSetViewController
...
- (void)viewDidLoad {
    ...
    // 取消网络质量检测(进入到邀请码页面，用户需要输入邀请码，故不需要网络质量检测 --- 需要用户在tcp竞速成功后，使用最新节点进行质量检测 --- 解决在用户输入邀请码过程中，网络质量检测导致的节点切换问题)
...
- (void)processData {
    ...
    self.blurView.clickLoginBtnAction = ^(ZSsoTypeMenu ssoType, NSString * _Nonnull ssoText) {
        ...
        if (ssoType == ZSsoTypeMenuCompanyId) {
            if (ssoText.length == 0) {
                [HUD showMessage:LanguageToolMatch(@"邀请码错误") inView:self.view];
                return;
            }
        ...
        switch (step) {
            case ZNetRacingStepOss:
            {
                ...
                if ([lastTwo isEqualToString:@"01"]) {
                    [self showErrorMessageWithMessage:[NSString stringWithFormat:@"%@%@",LanguageToolMatch(@"获取邀请码配置失败"),errorCode] errorCode:errorCode];
                } else {
                    //OSS
                    if (code == 100000) {
